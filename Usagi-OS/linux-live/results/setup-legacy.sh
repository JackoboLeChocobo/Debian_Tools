...
dev=`zenity \
	...
	--text="Choisissez le périphérique USB désiré" \
	--list \
	--column="Périphérique" \
	--column="Nom" \
...
if echo "$dev" | grep '/'; then
	...
	echo " "
	echo "Périphérique choisi : $dev"
	echo " "
	sleep 3

	apt install progress grub-pc-bin
	...
	grub-install --removable --boot-directory=/media/install/mnt/boot --target=i386-pc $dev

	uuid=`blkid -s UUID -o value ${dev}2`
	sleep 10
	
	echo "menuentry 'Usagi OS'{" > /media/install/mnt/boot/grub/grub.cfg
	echo "insmod part_msdos" >> /media/install/mnt/boot/grub/grub.cfg
	...
	echo "linux /Usagi-OS/boot/vmlinuz root=UUID=$uuid ro quiet splash perch" >> /media/install/mnt/boot/grub/grub.cfg
	echo "initrd /Usagi-OS/boot/initrfs.img" >> /media/install/mnt/boot/grub/grub.cfg
	echo "}" >> /media/install/mnt/boot/grub/grub.cfg

	mount ${dev}2 /media/install/datas
	echo "Copie des données en cours..."
	sleep 3
	cp -dR Usagi-OS /media/install/datas & progress -mp $!
